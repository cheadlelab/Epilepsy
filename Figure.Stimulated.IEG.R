...
# Clean y labels (remove "_Stimulated")
...
# Gene group for faceting
df_plot$GeneGroup <- ifelse(as.character(df_plot$Gene) %in% IEG, "IEG", "HSP")
df_plot$GeneGroup <- factor(df_plot$GeneGroup, levels = c("IEG","HSP"))

# Gene order
gene_levels <- c(IEG, HSP)
df_plot$Gene <- factor(as.character(df_plot$Gene), levels = gene_levels)
...
pp <- ggplot(df_plot, aes(x = Gene, y = group_clean, fill = l2fc)) +
  ...
  scale_x_discrete(position = "bottom") +   # put gene names below
  facet_grid(cols = vars(GeneGroup), scales = "free_x", space = "free_x") +
  theme_minimal(base_size = 7) +
  theme(
    axis.title.x   = element_blank(),  # remove "Genes"
    axis.title.y   = element_blank(),  # remove "Cell type"
    ...
    strip.text.x   = element_text(size = 7, face = "bold")  # keep IEG / HSP labels
  )

# Save
pdf(file = "//grid/cheadle_home/qianyu/Epilepsy/manuscript/V5/Figure.Stimulated.IEG.pdf", width = 4, height = 2.8)
print(pp)
dev.off()
