...
# Params
####################################################################################################
COX <- c("COX4I1", "COX5A", "COX5B", "COX6A1", "COX6B1", "COX7A2", "COX7C", "COX8A")
genes_of_interest <- COX
...
# NA tile color (missing Gene×celltype -> will be filled as NA)
...
# 3) Build heatmap df + plot (Stimulated_vs_Distal) with completed tiles and NA as grey
plot_stim_heatmap <- function(df_l2fc, sig_hits, genes_keep, cell_types,
  ...
  # IMPORTANT: complete the Gene×celltype matrix so missing combos become NA tiles
  df_plot <- df_l2fc %>%
  ...
  p <- ggplot(df_plot, aes(x = Gene, y = celltype, fill = l2fc)) +
    geom_tile(
...
# Run
####################################################################################################
# Keep genes that appear anywhere (best-effort), preserve order of genes_of_interest
genes_keep <- unique(intersect(
  genes_of_interest,
  unique(unlist(lapply(cell_types, function(ct) {
    g <- c()
    if (is.null(final_results[[ct]])) return(g)
    for (cmp in comparisons) {
      deg <- final_results[[ct]][[cmp]]$deg
...
# Save
####################################################################################################
pdf("//grid/cheadle_home/qianyu/Epilepsy/manuscript/V5/Figure.Stimulated.COX.pdf", width = 3.5, height = 3.2)
print(pp)
dev.off()
