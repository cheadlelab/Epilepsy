...
top_up <- df %>% filter(avg_log2FC > 0) %>% arrange(desc(avg_log2FC)) %>% slice_head(n = 20)
top_down <- df %>% filter(avg_log2FC < 0) %>% arrange(avg_log2FC) %>% slice_head(n = 5)
...
pdf(file = "//grid/cheadle_home/qianyu/Epilepsy/manuscript/V3/Figure.LARA.Vol.pdf", width = 2, height = 2)
print(p1)
...
top_up <- df %>% filter(avg_log2FC > 0) %>% arrange(desc(avg_log2FC)) %>% slice_head(n = 10)
top_down <- df %>% filter(avg_log2FC < 0) %>% arrange(avg_log2FC) %>% slice_head(n = 10)
top_lab <- bind_rows(top_up, top_down, force_row) %>% distinct(gene, .keep_all = TRUE)
...
pdf(file = "//grid/cheadle_home/qianyu/Epilepsy/manuscript/V3/Figure.RA.Vol.pdf", width = 2, height = 2)
print(p1)
dev.off() # Close the PDF device
...
pdf(file = "//grid/cheadle_home/qianyu/Epilepsy/manuscript/V5/Figure.RA.GO.pdf", width = 2, height = 1.5)
print(p1)
dev.off() # Close the PDF device
