...
# ieg module (expanded)
####################################################################################################
# A compact-but-expanded ieg set (cytosolic ieg70/ieg90 + co-chaperones + small ieg + key ER/mito chaperones)
IEG <- c("FOS", "FOSB", "JUN", "JUNB", "EGR1", "EGR2", "ARC", "NR4A1", "NR4A2", "NPAS4", "ATF3", "DUSP1", "DUSP6")


IEG <- rev(IEG)

# Build a long table defining gene -> group (ONLY IEG)
IEG_GROUPS <- tibble::tibble(
  GeneGroup = "IEG",
  Gene = IEG
) %>%
  dplyr::distinct()
...
plot_ieg_twocmp_heatmap <- function(df_deg, IEG_GROUPS, cell_types, cmp_vec,
  ...
  # Keep gene order EXACTLY as provided in IEG vector
  IEG_GROUPS_keep <- IEG_GROUPS %>%
    dplyr::mutate(
      GeneGroup = factor(GeneGroup, levels = c("IEG")),
      Gene_order = dplyr::row_number()
    )
  
  # Full grid: (all IEG genes) x (all cell types) x (both comparisons)
  df_grid <- IEG_GROUPS_keep %>%
    tidyr::expand_grid(
      celltype = cell_types,
      Comparison = cmp_vec
    )
  ...
  df_plot <- df_grid %>%
    ...
    dplyr::mutate(
      ...
      # enforce y-order
      Gene = factor(Gene, levels = IEG_GROUPS_keep$Gene)
    )
...
pp <- plot_ieg_twocmp_heatmap(
  df_deg = df_deg2,
  IEG_GROUPS = IEG_GROUPS,
...
# Save
####################################################################################################
out_pdf <- "//grid/cheadle_home/shared/Qianyu_shared/Epilepsy/manuscript/V6/Figure.Focal_vs_Distal__Stimulated_vs_Distal.IEG.side_by_side.pdf"

pdf(file = out_pdf, width = 7, height = 2.7, useDingbats = FALSE)
print(pp)
dev.off()
