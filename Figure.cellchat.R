...
netVisual_heatmap(
  cellchat.FD,
  measure = "weight",
  sources.use = c("Lipid-Accumulated Reactive Astrocyte", "Reactive Astrocyte", "Other Astrocyte", "Microglia","OPC","OxPos OL", "OL"),
  targets.use = c("Lipid-Accumulated Reactive Astrocyte", "Reactive Astrocyte", "Other Astrocyte", "Microglia","OPC","OxPos OL", "OL"),
  row.show    = c("Lipid-Accumulated Reactive Astrocyte", "Reactive Astrocyte", "Other Astrocyte", "Microglia","OPC","OxPos OL", "OL"),
  col.show    = c("Lipid-Accumulated Reactive Astrocyte", "Reactive Astrocyte", "Other Astrocyte", "Microglia","OPC","OxPos OL", "OL"),
  remove.isolate = TRUE,
  title.name = "Focal vs. Distal")
...
net.up <- subsetCommunication(cellchat, net = net, datasets = "Focal",ligand.logFC = 0.1, receptor.logFC = 0.1)
net.down <- subsetCommunication(cellchat, net = net, datasets = "Distal",ligand.logFC = -0.1, receptor.logFC = -0.1)
...
netVisual_chord_gene(object.list[["Focal"]],
                     ...
                     lab.cex = 0.8,
                     small.gap = 3.5,
                     title.name = paste0("Up-regulated signaling of Inh -> IT in ",names(object.list)[2], " vs.", names(object.list)[1]))

par(mfrow = c(1,1), xpd=TRUE) # 10x5
netVisual_chord_gene(object.list[["Distal"]],
                     ...
                     lab.cex = 0.8,
                     small.gap = 3.5,
                     title.name = paste0("Down-regulated signaling of Inh -> IT in ", names(object.list)[2], " vs.", names(object.list)[1]))
...
gene.up <- extractGeneSubsetFromPair(net.up, cellchat.SD)
gene.down <- extractGeneSubsetFromPair(net.down, cellchat.SD)
...
par(mfrow = c(1,1), xpd=TRUE) # 10x4.5
netVisual_chord_gene(object.list[["Distal"]],
                     targets.use = c("Exc_L23IT","Exc_L4IT","Exc_L5IT_GRIN3A","Exc_L6IT"),
                     sources.use = c("Inh_SST","Inh_PVALB","Inh_VIP"),
                     slot.name = 'net',
...
par(mfrow = c(1,1), xpd=TRUE) # 10x4.5
netVisual_chord_gene(object.list[["Focal"]],
                     sources.use = c("Exc_L23IT","Exc_L4IT","Exc_L5IT_GRIN3A","Exc_L6IT"),
                     targets.use = c("Inh_SST","Inh_PVALB","Inh_VIP"),
                     slot.name = 'net',
                     net = net.up,
                     lab.cex = 0.8,
                     small.gap = 3.5,
                     title.name = paste0("Up-regulated signaling of IT -> Inh in ",names(object.list)[2], " vs.", names(object.list)[1]))

par(mfrow = c(1,1), xpd=TRUE) # 10x5.5
netVisual_chord_gene(object.list[["Distal"]],
                     sources.use = c("Exc_L23IT","Exc_L4IT","Exc_L5IT_GRIN3A","Exc_L6IT"),
                     targets.use = c("Inh_SST","Inh_PVALB","Inh_VIP"),
                     slot.name = 'net',
                     ...
                     lab.cex = 0.8,
                     small.gap = 3.5,
                     title.name = paste0("Down-regulated signaling of IT -> Inh in ", names(object.list)[2], " vs.", names(object.list)[1]))


#######################################################################################################
# Stimu vs Distal, Exc -> Inh
# 15*10
pos.dataset = "Stimu"
# pos.dataset = "Stimu"
features.name = pos.dataset
...
netVisual_chord_gene(object.list[["Focal"]],
                     ...
                     lab.cex = 0.8,
                     small.gap = 3.5,
                     title.name = paste0("Up-regulated signaling of Glia -> Inh in ",names(object.list)[2], " vs.", names(object.list)[1]))
...
netVisual_chord_gene(object.list[["Distal"]],
                     ...
                     lab.cex = 0.8,
                     small.gap = 3.5,
                     title.name = paste0("Down-regulated signaling of Glia -> Inh in ", names(object.list)[2], " vs.", names(object.list)[1]))
...
netVisual_chord_gene(object.list[["Focal"]],
                     ...
                     lab.cex = 0.8,
                     small.gap = 3.5,
                     title.name = paste0("Up-regulated signaling of Glia -> Exc in ",names(object.list)[2], " vs.", names(object.list)[1]))
...
netVisual_chord_gene(object.list[["Distal"]],
                     ...
                     lab.cex = 0.8,
                     small.gap = 3.5,
                     title.name = paste0("Down-regulated signaling of Glia -> Exc in ", names(object.list)[2], " vs.", names(object.list)[1]))



#######################################################################################################
# Stimu vs Distal, Glia -> Inh
# 15*10
pos.dataset = "Stimu"
# pos.dataset = "Stimu"
features.name = pos.dataset
